...
export class SpinnigWheelService {
  ...
  getWheel = async () => {
    ...
    if (!spinnigWheel) throw new InternalServerErrorException('No active spinnig wheel found');
  ...
  update = async (updateSpinnigWheelDto: UpdateSpinnigWheelDto) => {
    ...
    if (!spinnigWheel) throw new BadRequestException('No active spinnig wheel found with this id');
