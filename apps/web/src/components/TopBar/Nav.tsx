...
const NavBar619 = () => {
  ...
  const navBarItems: CardNavItem[] = [
    ...
    },
    {
      ...
      links: [
        { label: "Instagram", href: "https://www.instagram.com/voltobahrain/?hl=en", ariaLabel: "Instagram",type:"a" },
  ...
  const handleScroll = useCallback(() => {
    ...
    setNavState(prev => {
      const isScrollingDown = currentScrollY > prev.lastScrollY && currentScrollY >= scrollThreshold;
  ...
  const getNavbarClasses = () => {
    const baseClasses = "fixed -m-1 top-0 left-0 right-0 z-50 w-full transition-transform duration-300 ease-in-out";
  ...
  return (
    <nav className={"fixed -m-1 top-0 left-0 right-0 z-[99] w-full transition-transform  bg-transparent duration-300 ease-in-out"}>
      <CardNav {...navProps} />
    </nav>
  );
};
