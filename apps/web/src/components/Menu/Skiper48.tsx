            ...
            <AnimatePresence mode="wait">
              {current === index && (
                <motion.div
                  ...
                  className="absolute bottom-0 left-2 space-y-1 flex h-[14%] w-full translate-y-full items-center justify-center p-2 text-center font-medium tracking-tight text-gray-700/50"
                >
                  <div className=" ">
                    <h2 className="text-black/70 text-lg md:text-2xl">
                      {img.title} - {img.price}
                    </h2>
                    <p className="text-xs md:text-sm">{img.description}</p>
                  </div>
                </motion.div>
              )}
            </AnimatePresence>
