...
export function EventCarousel() {
    ...
    return (
        ...
        <div className=' w-full flex   justify-center cursor-pointer' dir="ltr">
                ...
                <Carousel autoPlay stopOnHover swipeable showThumbs={false} showStatus={false} emulateTouch infiniteLoop interval={2000} className=' w-full  h-fit' >
                    {events.map((event) => (
                        <EventBanner key={event.id} name={event.name} type={event.type} img={event.img} description={event.description} />
                    ))}
                </Carousel>
